...
def track_bubbles_as_dataframe_fast(
        ...
        if len(matched_track_ids) == 1:
            ...
            def try_backtrack_match(cur_box, frame_idx, max_bt):
                ...
                if cur_r < last_r * 0.5 or cur_r > last_r * 2.0:
                    return None
                # 检查2.2：绝对变化约束
                if cur_r - last_r < -10:  # 半径不能变小超过10
                    return None
                # 检查3：小气泡额外约束
                if cur_r < 30:
                    max_small_distance = 30 * time_gap
                    if distance > max_small_distance:
                        return None
                return (chosen_tid, max_iou)
            # 遍历主循环里 “没有上一帧匹配” 的分支
            # ========== (3) 没有和上一帧匹配到 ==========
            #    尝试多帧回溯（支持拆分）
            single_result = try_backtrack_match(cur_box, frame_idx, current_max_backtrack)
    ...
    filled_df['merged_flag'] = filled_df['bubble_id'].isin(merged_track_ids).astype(int)
...
def visualize_bubble_tracking(bubble_df, min_duration=3,image_folder=None, output_folder=None):
    """
    ...
    # 计算每个轨迹的持续时间
    bubble_stats = bubble_df.groupby('bubble_id').agg({
        'frame_idx': ['min', 'max', 'count'],
        'filename': 'count'
    }).round(2)

    bubble_stats.columns = ['first_frame', 'last_frame', 'frame_count', 'file_count']
    bubble_stats['duration'] = bubble_stats['last_frame'] - bubble_stats['first_frame'] + 1

    # 过滤条件：
    # 1. 持续时间 >= min_duration
    # 2. 帧数 >= min_duration (防止单帧闪烁)
    valid_bubble_ids = bubble_stats[
        (bubble_stats['duration'] >= min_duration) &
        (bubble_stats['frame_count'] >= min_duration)
        ].index.tolist()    # 过滤掉无效轨迹
    filtered_df = bubble_df[bubble_df['bubble_id'].isin(valid_bubble_ids)].copy()

    print(f"📊 轨迹统计: 原始{len(bubble_stats)}条, 过滤后{len(valid_bubble_ids)}条")
    print(f"🗑️ 过滤掉{len(bubble_stats) - len(valid_bubble_ids)}条短轨迹")

    # 显示被过滤的轨迹信息（用于调试）
    # short_trajectories = bubble_stats[
    #     (bubble_stats['duration'] < min_duration) |
    #     (bubble_stats['frame_count'] < min_duration)
    #     ]
    # if not short_trajectories.empty:
    #     print("被过滤的短轨迹:")
    #     for bubble_id, row in short_trajectories.iterrows():
    #         print(f"  - 轨迹{bubble_id}: 持续{row['duration']}帧, 出现{row['frame_count']}次")

    # ========== 新增：检测并过滤闪烁轨迹 ==========
    def detect_flickering_trajectories(df, flicker_threshold=2):
        """
        检测闪烁轨迹：轨迹中断次数过多的轨迹
        """
        flickering_trajectories = []

        for bubble_id, group in df.groupby('bubble_id'):
            group = group.sort_values('frame_idx')
            frames = group['frame_idx'].values

            # 计算帧间隔（检测轨迹中断）
            frame_gaps = np.diff(frames)
            large_gaps = frame_gaps[frame_gaps > 1]  # 间隔大于1帧的缺口

            # 如果中断次数过多，认为是闪烁轨迹
            if len(large_gaps) >= flicker_threshold:
                flickering_trajectories.append(bubble_id)
                # print(f"⚠️ 检测到闪烁轨迹 {bubble_id}: {len(large_gaps)}次中断")

        return flickering_trajectories

    # 检测闪烁轨迹
    flickering_ids = detect_flickering_trajectories(filtered_df)
    if flickering_ids:
        # print(f"🗑️ 过滤掉{len(flickering_ids)}条闪烁轨迹")
        filtered_df = filtered_df[~filtered_df['bubble_id'].isin(flickering_ids)]
    # 按文件名分组处理
...
if __name__ == "__main__":
    result_df = read_pkl_dict(r"E:\zl\400mA\output\output_images\out\all_results.pkl")
    #iou阈值设置（0：0.2 10），（10：0.2,20），（30：0.2,20），（45：0.3,30）net(0.2,10)
    bubble_df= track_bubbles_as_dataframe_fast(result_df, iou_thresh=0.2,max_backtrack=30)
    # print(bubble_df.head())fenbi
...
#     print(bubble_track)
    # 可视化保存到 out/
    visualize_bubble_tracking(bubble_df, image_folder=r'E:\zl\400mA\output\output_images', output_folder=r'E:\zl\400mA\output\output_images/output')