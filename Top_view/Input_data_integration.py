...
def json2coco(labelme_path, save_coco_path):
    ...
    for file in train_path:
        # shutil.copy(file.replace("json", "jpg"), "%scoco/images/train2017/" % save_coco_path)
        img_name = file.replace('json', 'jpg')
        temp_img = cv2.imread(img_name)
        try:
            cv2.imwrite(
                "{}/images/train2017/{}".format(save_coco_path, img_name.split('\\')[-1].replace('png', 'jpg')),
                temp_img)
    ...
    for file in val_path:
        # shutil.copy(file.replace("json", "jpg"), "%scoco/images/val2017/" % save_coco_path)
        img_name = file.replace('json', 'jpg')
        temp_img = cv2.imread(img_name)
        try:
            cv2.imwrite(
                "{}/images/val2017/{}".format(save_coco_path, img_name.split('\\')[-1].replace('png', 'jpg')),
                temp_img)
        except Exception as e:
            print(e)
            print('Wrong Image:', img_name)
...
if __name__ == '__main__':
    labelme_path = r"E:\bubble_top\xxx_pic\train_set"
    saved_coco_path = r"E:\bubble_top\xxx_pic\train_set/train"
